...
Every package's `package-lock.json` is the authority on what a release installs: release workflows install with `npm ci`, which fails when a lockfile no longer records the resolution its `package.json` asks for. [`scripts/lockfiles.js`](scripts/lockfiles.js) walks the same dependency order as the build driver and keeps them honest -- `npm run lockfiles:check` from the repository root reports every lockfile that has fallen out of step and leaves the tree untouched, and `npm run lockfiles:sync` regenerates them. Run the sync after any edit to a `package.json` dependency list.
